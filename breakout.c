...
void OnTick()
  {
    ...
    double comparisonHigh = iHigh(_Symbol,Timeframe,iHighest(_Symbol,Timeframe,MODE_HIGH,MinRangeBars,1));
    double comparisonLow = iLow(_Symbol,Timeframe,iLowest(_Symbol,Timeframe,MODE_LOW,MinRangeBars,1));
    ...
    if(setup.time1 <= 0){
       if(rangeBarsSize < comparisonBarsSize * MinRangeFactor){
          ...
          setup.timex = iTime(_Symbol,Timeframe,MinRangeBars+1);
          setup.high = rangeHigh;
          setup.low = rangeLow;
          setup.drawRect();
       }
    }else{
